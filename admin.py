...
## Default JSON file that stores the library data.
...
def find_book(books, search_text):
    ...
    # 1. Exact, case-insensitive match on the book ID.
    for book_id, book in books.items():
        if str(book_id).strip().lower() == query:
            return book_id

    # 2. Match on the book title (exact or partial).
    for book_id, book in books.items():
        title = str(book.get("title", "")).strip().lower()
        if query in title:
            return book_id

    # 3. Match on the book author (exact or partial).
    for book_id, book in books.items():
